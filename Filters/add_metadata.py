...
class Filter:
    ...
    async def inlet(
        ...
        if self.user_valves.surnom:
            system_message += f"Tu peux l'appeler : {', '.join([x.strip() for x in self.user_valves.surnom.split(',')])} en fonction du contexte.\n"
        if self.user_valves.autres_infos:
            system_message += f"Autres informations entrée par l'utilisateur: {self.user_valves.autres_infos}\n"
